...
const ChainBuster = () => {
  ...
  // Inputs for CHAINBUSTER table
  const [chainBusterDevSolBuy, setChainBusterDevSolBuy] = useState('');
  const [chainBusterWalletSolBuy, setChainBusterWalletSolBuy] = useState('');
  const [chainBusterSellJitoTip, setChainBusterSellJitoTip] = useState('0.1');
  // Wallets state
  ...
  const handleLaunch = async () => {
    ...
    setIsLoading(true);
    setLoadingMessage('Launching token with ChainBuster...');
    ...
    try {
      ...
      console.log('Launching token with metadata:', {
        ...
        devBuyAmount: Number(chainBusterDevSolBuy) || 0.05,
        chainBusterBuyAmount: Number(chainBusterWalletSolBuy) || 0.005,
        wallet1BuyAmount: 0.005,
        chainBusterSellJitoTip: Number(chainBusterSellJitoTip) || 0.001
      });

      // Call the new launch endpoint
      const response = await axios.post('http://localhost:5000/api/launch/chainbuster', {
  ...
  const handleFundSnipers = async () => {
    ...
    try {
      const response = await axios.post('http://localhost:5000/api/launch/chainbuster/fund-snipers', {
            ...
            <Table size="medium" sx={{ background: 'rgba(255,255,255,0.01)', borderRadius: 2, mb: 3, width: '100%' }}>
              ...
              <TableBody>
                <TableRow>
                  ...
                  <TableCell sx={{ color: 'white', borderBottom: 'none', fontSize: 16, py: 1 }}>
                    <TextField
                      value={chainBusterDevSolBuy}
                      onChange={e => setChainBusterDevSolBuy(e.target.value)}
                      size="small"
                ...
                <TableRow>
                  <TableCell sx={{ color: 'white', borderBottom: 'none', fontSize: 16, py: 1 }}>ChainBuster Wallet</TableCell>
                  <TableCell sx={{ color: 'white', borderBottom: 'none', fontSize: 16, py: 1 }}>57.005</TableCell>
                  <TableCell sx={{ color: 'white', borderBottom: 'none', fontSize: 16, py: 1 }}>
                    <TextField
                      value={chainBusterWalletSolBuy}
                      onChange={e => setChainBusterWalletSolBuy(e.target.value)}
                      size="small"
                      sx={{ input: { color: 'white', textAlign: 'center', fontSize: 16 }, background: 'rgba(255,255,255,0.03)', borderRadius: 1, width: 180 }}
                    />
                  </TableCell>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 3, mt: 3 }}>
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                <Typography sx={{ color: 'white', fontWeight: 500, fontSize: 17 }}>
                  ChainBuster Sell Jito Tip:
                </Typography>
                <TextField
                  value={chainBusterSellJitoTip}
                  onChange={e => setChainBusterSellJitoTip(e.target.value)}
                  size="small"
