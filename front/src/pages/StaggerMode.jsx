...
const StaggerMode = () => {
  ...
  // Inputs for STAGGER MODE table
  const [staggerDevSolBuy, setStaggerDevSolBuy] = useState('');
  const [staggerWalletSolBuy, setStaggerWalletSolBuy] = useState('');
  const [staggerSellJitoTip, setStaggerSellJitoTip] = useState('0.1');
  // Stagger mode selection
  const [staggerMode, setStaggerMode] = useState('automatic');
  // Wallets state
  ...
  const handleLaunch = async () => {
    ...
    setIsLoading(true);
    setLoadingMessage('Launching token with Stagger Mode...');
    ...
    try {
      ...
      console.log('Launching token with metadata:', {
        ...
        devBuyAmount: Number(staggerDevSolBuy) || 0.05,
        staggerBuyAmount: Number(staggerWalletSolBuy) || 0.005,
        wallet1BuyAmount: 0.005,
        staggerSellJitoTip: Number(staggerSellJitoTip) || 0.001,
        staggerMode: staggerMode
      });

      // Call the new launch endpoint
      const response = await axios.post('http://localhost:5000/api/launch/stagger-mode', {
  ...
  const handleFundSnipers = async () => {
    ...
    try {
      const response = await axios.post('http://localhost:5000/api/launch/stagger-mode/fund-snipers', {
          ...
          <Paper elevation={3} sx={{ 
            ...
            <Box sx={{ display: 'flex', justifyContent: 'center', mt: 4, mb: 3 }}>
              <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
                <Typography variant="h6" sx={{ color: 'white', fontWeight: 600, fontSize: 20 }}>
                  Stagger Mode Selection
                </Typography>
                <ToggleButtonGroup
                  value={staggerMode}
                ...
                >
                  <ToggleButton value="automatic" sx={{ px: 4, py: 2, fontSize: 16, fontWeight: 600 }}>
                    Automatic
                  </ToggleButton>
                  <ToggleButton value="manual" sx={{ px: 4, py: 2, fontSize: 16, fontWeight: 600 }}>
                    Manual
                  </ToggleButton>
                </ToggleButtonGroup>
                <Typography variant="body2" sx={{ color: 'rgba(255,255,255,0.7)', textAlign: 'center', maxWidth: 400 }}>
                  {staggerMode === 'automatic' 
                    ? 'Automatic: Stagger will start immediately after dev buy'
                    : 'Manual: Start stagger yourself in Quick Actions on Sell page'
                  }
                </Typography>
              </Box>
            </Box>
            {/* Add the Fund Snipers button below the Stagger Mode Selection */}
            <Box sx={{ display: 'flex', justifyContent: 'center', mt: 4 }}>
            ...
            <Table size="medium" sx={{ background: 'rgba(255,255,255,0.01)', borderRadius: 2, mb: 3, width: '100%' }}>
              ...
              <TableBody>
                <TableRow>
                  ...
                  <TableCell sx={{ color: 'white', borderBottom: 'none', fontSize: 16, py: 1 }}>
                    <TextField
                      value={staggerDevSolBuy}
                      onChange={e => setStaggerDevSolBuy(e.target.value)}
                      size="small"
                ...
                <TableRow>
                  <TableCell sx={{ color: 'white', borderBottom: 'none', fontSize: 16, py: 1 }}>Stagger Wallet</TableCell>
                  <TableCell sx={{ color: 'white', borderBottom: 'none', fontSize: 16, py: 1 }}>57.005</TableCell>
                  <TableCell sx={{ color: 'white', borderBottom: 'none', fontSize: 16, py: 1 }}>
                    <TextField
                      value={staggerWalletSolBuy}
                      onChange={e => setStaggerWalletSolBuy(e.target.value)}
                      size="small"
                      sx={{ input: { color: 'white', textAlign: 'center', fontSize: 16 }, background: 'rgba(255,255,255,0.03)', borderRadius: 1, width: 180 }}
                    />
                  </TableCell>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 3, mt: 3 }}>
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                <Typography sx={{ color: 'white', fontWeight: 500, fontSize: 17 }}>
                  Stagger Sell Jito Tip:
                </Typography>
                <TextField
                  value={staggerSellJitoTip}
                  onChange={e => setStaggerSellJitoTip(e.target.value)}
                  size="small"
