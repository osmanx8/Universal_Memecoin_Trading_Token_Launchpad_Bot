...
const ManualMode = () => {
  ...
  // Inputs for MANUAL MODE table
  const [manualDevSolBuy, setManualDevSolBuy] = useState('');
  const [manualWalletSolBuy, setManualWalletSolBuy] = useState('');
  const [manualSellJitoTip, setManualSellJitoTip] = useState('0.1');
  // Wallets state
  ...
  const handleLaunch = async () => {
    ...
    setIsLoading(true);
    setLoadingMessage('Launching token with Manual Mode...');
    ...
    try {
      ...
      console.log('Launching token with metadata:', {
        ...
        devBuyAmount: Number(manualDevSolBuy) || 0.05,
        manualBuyAmount: Number(manualWalletSolBuy) || 0.005,
        wallet1BuyAmount: 0.005,
        manualSellJitoTip: Number(manualSellJitoTip) || 0.001
      });

      // Call the new launch endpoint
      const response = await axios.post('http://localhost:5000/api/launch/manual-mode', {
  ...
  const handleFundSnipers = async () => {
    ...
    try {
      const response = await axios.post('http://localhost:5000/api/launch/manual-mode/fund-snipers', {
            ...
            <Table size="medium" sx={{ background: 'rgba(255,255,255,0.01)', borderRadius: 2, mb: 3, width: '100%' }}>
              ...
              <TableBody>
                <TableRow>
                  ...
                  <TableCell sx={{ color: 'white', borderBottom: 'none', fontSize: 16, py: 1 }}>
                    <TextField
                      value={manualDevSolBuy}
                      onChange={e => setManualDevSolBuy(e.target.value)}
                      size="small"
                ...
                <TableRow>
                  <TableCell sx={{ color: 'white', borderBottom: 'none', fontSize: 16, py: 1 }}>Manual Wallet</TableCell>
                  <TableCell sx={{ color: 'white', borderBottom: 'none', fontSize: 16, py: 1 }}>57.005</TableCell>
                  <TableCell sx={{ color: 'white', borderBottom: 'none', fontSize: 16, py: 1 }}>
                    <TextField
                      value={manualWalletSolBuy}
                      onChange={e => setManualWalletSolBuy(e.target.value)}
                      size="small"
                      sx={{ input: { color: 'white', textAlign: 'center', fontSize: 16 }, background: 'rgba(255,255,255,0.03)', borderRadius: 1, width: 180 }}
                    />
                  </TableCell>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 3, mt: 3 }}>
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                <Typography sx={{ color: 'white', fontWeight: 500, fontSize: 17 }}>
                  Manual Sell Jito Tip:
                </Typography>
                <TextField
                  value={manualSellJitoTip}
                  onChange={e => setManualSellJitoTip(e.target.value)}
                  size="small"
