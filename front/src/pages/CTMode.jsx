...
const CTMode = () => {
  ...
  // Inputs for CTO MODE table
  const [ctDevSolBuy, setCtDevSolBuy] = useState('');
  const [ctWalletSolBuy, setCtWalletSolBuy] = useState('');
  const [ctSellJitoTip, setCtSellJitoTip] = useState('0.1');
  // Wallets state
  ...
  const handleLaunch = async () => {
    ...
    setIsLoading(true);
    setLoadingMessage('Launching token with CTO Mode...');
    ...
    try {
      ...
      console.log('Launching token with metadata:', {
        ...
        devBuyAmount: Number(ctDevSolBuy) || 0.05,
        ctBuyAmount: Number(ctWalletSolBuy) || 0.005,
        wallet1BuyAmount: 0.005,
        ctSellJitoTip: Number(ctSellJitoTip) || 0.001
      });

      // Call the new launch endpoint
      const response = await axios.post('http://localhost:5000/api/launch/cto-mode', {
  ...
  const handleFundSnipers = async () => {
    ...
    try {
      const response = await axios.post('http://localhost:5000/api/launch/cto-mode/fund-snipers', {
          ...
          <Paper elevation={3} sx={{ 
              ...
              <TableBody>
                <TableRow>
                  ...
                  <TableCell sx={{ color: 'white', borderBottom: 'none', fontSize: 16, py: 1 }}>
                    <TextField
                      value={ctDevSolBuy}
                      onChange={e => setCtDevSolBuy(e.target.value)}
                      size="small"
                ...
                <TableRow>
                  <TableCell sx={{ color: 'white', borderBottom: 'none', fontSize: 16, py: 1 }}>CTO Wallet</TableCell>
                  <TableCell sx={{ color: 'white', borderBottom: 'none', fontSize: 16, py: 1 }}>57.005</TableCell>
                  <TableCell sx={{ color: 'white', borderBottom: 'none', fontSize: 16, py: 1 }}>
                    <TextField
                      value={ctWalletSolBuy}
                      onChange={e => setCtWalletSolBuy(e.target.value)}
                      size="small"
                      sx={{ input: { color: 'white', textAlign: 'center', fontSize: 16 }, background: 'rgba(255,255,255,0.03)', borderRadius: 1, width: 180 }}
                    />
                  </TableCell>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 3, mt: 3 }}>
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                ...
                <TextField
                  value={ctSellJitoTip}
                  onChange={e => setCtSellJitoTip(e.target.value)}
                  size="small"
